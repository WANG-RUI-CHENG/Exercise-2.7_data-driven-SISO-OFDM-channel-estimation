...
def build_ce_dnn(K, SNR, savefile, learning_rate=1e-3, training_epochs=2000,
    ...
    val_ls, val_labels, val_Yp, val_Xp = sample_gen(
        batch_size * 100, SNR, training_flag=False, NoCP=(not cp_flag), CP_flag=cp_flag
    )
    for epoch in range(training_epochs + 1):
        train_loss = 0.
        for _ in range(20):
            batch_ls, batch_labels, Yp, Xp = sample_gen(
                batch_size, SNR, training_flag=True, NoCP=(not cp_flag), CP_flag=cp_flag
            )
            sample = np.concatenate((Yp, Xp), axis=1).astype(np.float32)
            labels = batch_labels.astype(np.float32)
            _, loss = sess.run([optimizer, loss_], feed_dict={nn_input: sample, H_true: labels})
            train_loss += loss
        sys.stdout.write('\repoch={epoch:<6d} loss={loss:.9f} on train set'.format(
        ...
        if epoch % test_step == 0:
            ...
            loss = sess.run(loss_, feed_dict={nn_input: sample, H_true: labels})
            if np.isnan(loss):
